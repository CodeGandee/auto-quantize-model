...
YOLO11_REPO_URL="$(yq -r '.source.repo_url' "${CFG}")"
...
Bootstrap YOLO11 assets (source + checkpoints).
...
if $CLEAN_ONLY; then
  echo "Cleaning temporary directory ${YOLO11_ROOT}/${TMP_SUBDIR}"
  rm -rf -- "${YOLO11_ROOT}/${TMP_SUBDIR}" || true
  exit 0
fi
...
if [[ -z "${ASSETS_DIR}" ]]; then
  ...
  if [[ -n "${SAVED_ASSETS_DIR}" ]]; then
    ...
    CANDIDATE="$(cd "${YOLO11_ROOT}/${DEFAULT_ASSETS_DIR_REL}" && pwd)"
  fi
...
if [[ -z "${ASSETS_DIR}" ]]; then
  echo "Enter the directory where YOLO11 assets should be stored (will create if missing)."
  read -r -p "YOLO11 assets dir: " ASSETS_DIR
fi
...
REPO_SRC_PATH="${YOLO11_ROOT}/${SRC_SUBDIR}"
REPO_CHECKPOINTS_PATH="${YOLO11_ROOT}/${CHECKPOINTS_SUBDIR}"
REPO_TMP_PATH="${YOLO11_ROOT}/${TMP_SUBDIR}"

ensure_repo_path() {
  local repo_path="$1"
  local physical_path="$2"

  if [[ "${ASSETS_DIR}" = "${YOLO11_ROOT}" ]]; then
    return 0
  fi
...
echo "[YOLO11] Assets dir:      ${ASSETS_DIR}"
echo "[YOLO11] Source dir:      ${SRC_DIR}"
echo "[YOLO11] Checkpoints dir: ${CHECKPOINTS_DIR}"
...
echo "[YOLO11] Recording selected assets dir into ${CFG}..."
