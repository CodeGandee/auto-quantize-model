...
def _sanitize_text(text: str) -> str:
    # Replace obvious absolute paths without trying to fully parse Markdown.
    text = re.sub(r"(?P<path>/[A-Za-z0-9._\\-~/]+)", "<ABSOLUTE_PATH>", text)
    return text
...
def main(argv: list[str]) -> int:
    if len(argv) != 3:
        print("Usage: sanitize_artifacts.py <run_output_dir> <expected_case_dir>", file=sys.stderr)
        return 2
    ...
    # Prefer the standardized filenames where possible.
    ...
    # Copy (and sanitize) any quant manifest JSON in this run dir.
