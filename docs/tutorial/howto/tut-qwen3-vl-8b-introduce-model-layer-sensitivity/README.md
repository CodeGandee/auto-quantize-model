...
This tutorial pack introduces `Qwen3-VL-8B-Instruct` into this repo (via a local
checkpoint symlink) and runs a reproducible per-layer sensitivity analysis using
NVIDIA ModelOpt AutoQuant.
...
The all-layers driver is parameterized by `--model-dir`, so it can run with
`Qwen3-VL-8B-Instruct` as long as `--model-dir` points at the 8B checkpoint.
The path still contains “4b” because it was originally introduced for the 4B
model, but it is model-agnostic at runtime.
