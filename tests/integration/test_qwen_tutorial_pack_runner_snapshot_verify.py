...
def test_snapshot_cleanup_removes_stale_scenarios_and_files(tmp_path: Path) -> None:
    ...
    output_dir = tmp_path / "output_out"
    _write_summary_pair(output_dir, suffix="selected")

    snapshot_scenario(expected_report_dir, selected, output_dir)
    ...
    assert sorted(child.name for child in selected_case_dir.iterdir()) == ["summary.json"]
