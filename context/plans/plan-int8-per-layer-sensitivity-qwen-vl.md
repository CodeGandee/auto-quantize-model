...
5. **Extend Qwen3-VL all-layers helper for INT8**:
   - Add an optional `--quant-format` or `--scheme-type` flag to `run_qwen3_vl_4b_autoquant_all_layers_fp8.py` (or a new sibling script) to switch between `FP8_ALL_LAYERS_CFG` and `INT8_ALL_LAYERS_CFG`.
...
- [ ] **Add INT8 configs in ModelOpt wrapper** Implement `INT8_LM_DEFAULT_CFG` and `INT8_ALL_LAYERS_CFG` in `src/auto_quantize_model/modelopt_configs.py` and expose them via `CUSTOM_QUANT_CONFIGS`.
- [ ] **Extend Qwen2.5-VL LM-only AutoQuant for INT8** Update `qwen2_5_vl_3b_autoquant_fp8_schemes.py` to add INT8 schemes, reuse common AutoQuant wiring, and ensure INT8 runs emit `per-layer-sensitivity.{md,json}`.
- [ ] **Add Qwen2.5-VL all-layers INT8 variant** Extend or clone `qwen2_5_vl_3b_autoquant_fp8_all_layers_per_scheme.py` to drive INT8 all-layers schemes using `INT8_ALL_LAYERS_CFG`, writing per-scheme Markdown + JSON.
- [ ] **Generalize Qwen3-VL all-layers helper for INT8** Update `run_qwen3_vl_4b_autoquant_all_layers_fp8.py` (or add a sibling script) so it can run INT8 all-layers analysis with the same output structure.
- [ ] **Implement Qwen3-VL LM-only INT8 driver** Add `run_qwen3_vl_4b_autoquant_int8_lm.py` that follows the Qwen2.5 LM-only pattern with text-only calibration data and writes `tmp/qwen3_vl_4b_autoquant_int8_lm/per-layer-sensitivity.{md,json}`.
- [ ] **Wire calibration defaults and CLI flags** Ensure each driver has sensible defaults for `--max-calib-samples`, `--auto-quantize-score-size`, and model paths, and that FP8 vs INT8 schemes are selectable via CLI.
- [ ] **Run sanity INT8 tests** In the `rtx5090` env, run each INT8 driver with small sample counts to confirm manifests and per-layer sensitivity outputs are generated without errors.
