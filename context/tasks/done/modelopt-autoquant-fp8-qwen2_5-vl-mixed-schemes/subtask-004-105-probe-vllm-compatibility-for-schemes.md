...
- [ ] **CANCEL** Job-004-105-001 Review `scripts/qwen/run_qwen2_5_vl_3b_vllm_fp8.py` to understand how it currently loads the FP8 LM-only checkpoint and how model paths and quantization settings are specified.
- [ ] **CANCEL** Job-004-105-002 Extend the script (or add a new helper) to accept either a `--scheme-name` argument or an explicit `--model-dir` pointing to a scheme-specific checkpoint.
- [ ] **CANCEL** Job-004-105-003 Implement a loop or small driver that attempts to load each scheme’s checkpoint in vLLM, capturing success/failure and any exceptions.
- [ ] **CANCEL** Job-004-105-004 Define and implement a structured output format (e.g., JSON or Markdown table) that records compatibility results per scheme and saves it under `models/qwen2_5_vl_3b_instruct/reports/` or `tmp/`.
- [ ] **CANCEL** Job-004-105-005 Run the compatibility checks for all schemes on the RTX 5090 environment and store the resulting logs and summary artifacts.
