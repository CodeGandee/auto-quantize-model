...
- For this Qwen2.5-VL-3B INC sensitivity and W8A8 spectrum work, we **choose the PyTorch-INC path on the native HF model** and treat ONNX export as a later deployment step:
  ...
  - INC’s PyTorch adaptor already supports generic MSE-based per-op sensitivity (`calculate_op_sensitivity`) and HAWQ_V2 flows for LLM-style models, as described in `context/summaries/inc-kb/howto-inc-layer-sensitivity-for-mixed-precision.md`.
