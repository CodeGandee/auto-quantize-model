...
- [ ] Job-001-102-001 Create `conf/quant_granularity/default.yaml` with a consistent schema and `quant_cfg_overrides: {}`.
- [ ] Job-001-102-002 Add `conf/quant_granularity/recipe_match_channel_token.yaml` (or final chosen name) encoding the Qwen3 target recipe granularity mapping from Subtask 1.1.
- [ ] Job-001-102-003 Add 2–4 additional overlays for sweep coverage (based on Subtask 1.1 decisions), for example:
  - `w_per_out_channel`, `w_per_in_channel`
  - `w_group64`, `w_group128`
  - `a_per_token_dynamic` (if needed as a standalone overlay)
- [ ] Job-001-102-004 Update `conf/preset/qwen3_lm_sensitivity.yaml` to include `quant_granularity: default` in defaults composition.
- [ ] Job-001-102-005 Add a short README note (either in the main task file or in `models/qwen3_vl_4b_instruct/layer-analysis/README.md`) describing the purpose and schema of `conf/quant_granularity/` (full usage docs are in Subtask 1.6).
