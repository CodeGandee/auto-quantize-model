...
- [x] Job-001-103-001 Choose the code location for the helper:
...
- [x] Job-001-103-002 Implement `apply_quant_cfg_overrides(base_cfg: dict, overrides: dict) -> dict` (exact signature TBD) using a deep copy and a narrow, explicit merge strategy.
- [x] Job-001-103-003 Implement axis/block_sizes mutual exclusion logic:
...
- [x] Job-001-103-004 Implement `block_sizes` key normalization:
...
- [x] Job-001-103-005 Decide and implement `default` propagation behavior:
  - If a config uses `quant_cfg["default"]`, decide whether overlays should also update it to match weight/input overrides.
- [x] Job-001-103-006 Add clear validation errors for bad overlays (unknown top-level keys, incompatible values, unsupported structures).

## Implementation notes

- Implemented in `src/auto_quantize_model/modelopt_quant_overrides.py`.
- Supported overlay keys: `"*weight_quantizer"`, `"*input_quantizer"`, and `"default"`.
- `block_sizes` overlays **merge** into an existing `block_sizes` mapping (so special keys like `type` / `scale_bits` are preserved unless explicitly overridden).
- Default propagation decision: overlays only apply to keys explicitly present in `quant_cfg_overrides` (no implicit propagation from weight/input into `default`).
