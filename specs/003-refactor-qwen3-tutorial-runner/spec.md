...
- Q: In snapshot mode, should `expected_report/` contain only `summary.*` or additional sanitized artifacts? → A: Snapshot only `summary.json` + `summary.md` per scenario.
...
- **Expected report snapshot**: The tracked, per-scenario `summary.json` + `summary.md` used for verification.
...
- **FR-008**: The runner MUST produce schema-locked, sanitized summaries for each scenario in both machine-readable and human-readable formats.  
  **Acceptance**: For each requested scenario, both `summary.json` and `summary.md` are generated and remain compatible with the existing summary schema contract.
- **FR-009**: The runner MUST support a snapshot workflow that refreshes the expected report snapshot for selected scenarios and removes stale expected scenarios not selected.  
  **Acceptance**: After snapshot mode, the expected report directory contains exactly the selected scenarios and only `summary.json` + `summary.md` for each scenario.
...
- **Sanitized Summary**: The stable, reviewable artifacts used for snapshotting and verification (`summary.json`, `summary.md`).
- **Expected Report Snapshot**: The tracked reference set of per-scenario `summary.json` + `summary.md` used as the “golden” verification target.
